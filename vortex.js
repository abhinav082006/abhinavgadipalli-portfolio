/* =========================================================
   Vortex — ported from Originkit's React/Three.js component.
   The engine (createVortex) is framework-agnostic already; only the
   React wrapper (props/hooks) has been replaced with a plain init
   using the preset values from the pasted component:
     background:#000000, topRadius:380, waistRadius:53, waistPosition:50,
     bottomRadius:1150, twist:3, zoom:75, speed:10, direction:'right',
     dots:true, comets:true — everything else at component defaults.
   ========================================================= */
(function () {
  if (typeof THREE === 'undefined') {
    console.warn('[Vortex] THREE not loaded — skipping vortex init.');
    return;
  }

  const canvas = document.getElementById('vortex-canvas');
  const container = canvas ? canvas.closest('.section-inner') : null;
  ...
  function createVortex(canvas, container, cfgRef) {
    ...
    function addWave(x, y, z, at, amp) {
      ...
      for (let i = 0; i < MAX_WAVES; i++) {
        ...
        if (waves[i].at < oldest) { oldest = waves[i].at; slot = i; }
      }
    ...
    function waveAt(x, y, z, now, out, at, radius) {
      ...
      for (let i = 0; i < MAX_WAVES; i++) {
        ...
        const age = now - w.at;
        if (age > WAVE_LIFE) { w.active = false; continue; }
        any = true;
    ...
    function burstAt(i, now, force) {
      rippleAwake = true;
      const x = dotHome[i * 3], y = dotHome[i * 3 + 1], z = dotHome[i * 3 + 2];
      ...
      for (let j = 0; j < dotCount; j++) {
        ...
        if (swell > dotScale[j]) { dotScale[j] = swell; dotScaleVel[j] = 0; }
      }
    }
    ...
    function drawStrand(strand, now, entrance) {
      ...
      for (let i = 0; i < STRAND_SEGMENTS - 1; i++) {
        const a = i * 3;
        const b = (i + 1) * 3;
        strandPos[w] = pts[a]; strandPos[w + 1] = pts[a + 1]; strandPos[w + 2] = pts[a + 2];
        strandCol[w] = cols[a]; strandCol[w + 1] = cols[a + 1]; strandCol[w + 2] = cols[a + 2];
        w += 3;
        strandPos[w] = pts[b]; strandPos[w + 1] = pts[b + 1]; strandPos[w + 2] = pts[b + 2];
        strandCol[w] = cols[b]; strandCol[w + 1] = cols[b + 1]; strandCol[w + 2] = cols[b + 2];
        w += 3;
      }
    }
    ...
    function collide(now) {
      ...
      for (const comet of cometList) {
        ...
        const x = comet.trail[0], y = comet.trail[1], z = comet.trail[2];
  ...
  /* ============================================================ mount */
  // Preset values from the pasted component's __originkitPresetProps,
  // with everything else at component defaults.
  const topRadius = 380, waistRadius = 53, waistPosition = 50, bottomRadius = 1150;
  const twist = 3, zoom = 75, speed = 10, direction = 'right';
  const lineOpt = { count: 240, color: '#B98356', glow: 10 };
  const dotOpt = { count: 8000, size: 20, color: '#E8C9A0', glow: 10, flicker: 10 };
  const cometOpt = { count: 10, speed: 6, color: '#F2A65A', glow: 6, tail: 19, delay: 8, collide: 6 };
  const repelOpt = { radius: 90, strength: 22 };
  ...
  const config = {
    ...
    lineColor: lineOpt.color,
    lineGlow: (lineOpt.glow / 10) * LINE_GLOW_MAX,
    showDots: true,
    dotCount: dotOpt.count,
    dotSize: dotOpt.size / DOT_SIZE_SCALE,
    dotColor: dotOpt.color,
    dotGlow: (dotOpt.glow / 10) * DOT_GLOW_MAX,
    dotFlicker: dotOpt.flicker / 10,
    showComets: true,
    cometCount: cometOpt.count,
    cometSpeed: (cometOpt.speed / 10) * COMET_SPEED_MAX,
    cometColor: cometOpt.color,
    cometGlow: (cometOpt.glow / 10) * COMET_GLOW_MAX,
    cometTail: cometOpt.tail,
    cometDelay: cometOpt.delay,
    collideForce: cometOpt.collide / 10,
    hoverRepel: true, // interactive — pointer pushes the strands/dots away
    repelRadius: repelOpt.radius,
    repelStrength: repelOpt.strength,
    running: !(reducedMotionQuery && reducedMotionQuery.matches),
  };
  ...
  if (reducedMotionQuery) {
    reducedMotionQuery.addEventListener('change', (e) => {
      cfgRef.current.running = !e.matches;
    });
  }
